...
class TestDrumGeneratorClassDiagram:
    """SC2: arch report includes class diagrams."""

    def test_class_diagram_present_in_arch_report(
        self, drum_repos: tuple[Path, Path, Path]
    ) -> None:
        """Class diagrams are rendered in the arch report."""
        _, generator, _ = drum_repos
        report = run_arch_review([generator], skip_llm=True)
        class_diagrams = [d for d in report.diagrams if d.scope == "classes"]
        assert len(class_diagrams) >= 1, (
            f"Expected at least 1 class diagram in arch report, got {len(class_diagrams)}"
        )
