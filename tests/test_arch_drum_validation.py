...
class TestDrumGeneratorClassDiagram:
    """SC2: arch on DrumGenerator produces a class diagram with key classes."""

    def test_class_diagram_present(self, drum_repos: tuple[Path, Path, Path]) -> None:
        _, generator, _ = drum_repos
        report = run_arch_review([generator], skip_llm=True)
        class_diagrams = [d for d in report.diagrams if d.scope == "classes"]
        assert len(class_diagrams) >= 1, "No class diagram generated"

    def test_class_diagram_has_processor(self, drum_repos: tuple[Path, Path, Path]) -> None:
        _, generator, _ = drum_repos
        report = run_arch_review([generator], skip_llm=True)
        class_diagrams = [d for d in report.diagrams if d.scope == "classes"]
        assert class_diagrams, "No class diagram generated"
        mermaid = class_diagrams[0].mermaid
        assert "DrumProcessor" in mermaid, f"DrumProcessor not in class diagram:\n{mermaid}"

    def test_class_diagram_has_controller(self, drum_repos: tuple[Path, Path, Path]) -> None:
        _, generator, _ = drum_repos
        report = run_arch_review([generator], skip_llm=True)
        class_diagrams = [d for d in report.diagrams if d.scope == "classes"]
        assert class_diagrams, "No class diagram generated"
        mermaid = class_diagrams[0].mermaid
        assert "Controller" in mermaid, f"Controller not in class diagram:\n{mermaid}"

    def test_class_diagram_shows_inheritance(
        self, drum_repos: tuple[Path, Path, Path]
    ) -> None:
        _, generator, _ = drum_repos
        report = run_arch_review([generator], skip_llm=True)
        ...
        assert class_diagrams, "No class diagram generated"
        mermaid = class_diagrams[0].mermaid
        assert "AudioProcessor <|-- DrumProcessor" in mermaid, (
            f"Missing inheritance arrow:\n{mermaid}"
        )
