...
class TestBasicParsing:
    def test_returns_evidence_for_each_java_file(
        self, collector: JavaAstCollector
    ) -> None:
    ...
    def test_payload_has_required_keys(
        ...
        results = collector.collect(FIXTURES, config=None)
        required_keys = {
            "file_path", "classes", "methods", "catch_blocks",
            "imports", "thread_pool_constructions", "log_statements",
        }
        for ev in results:
...
class TestHealthController:
    def test_class_annotations(self, collector: JavaAstCollector) -> None:
        results = collector.collect(FIXTURES, config=None)
        health = next(
            e for e in results if "HealthController" in e.payload["file_path"]
        )
...
class TestOrderService:
    def test_catch_block_swallowed(self, collector: JavaAstCollector) -> None:
        results = collector.collect(FIXTURES, config=None)
        order = next(
            e for e in results if "OrderService" in e.payload["file_path"]
        )
    ...
    def test_imports_include_resttemplate(
        self, collector: JavaAstCollector
    ) -> None:
        results = collector.collect(FIXTURES, config=None)
        order = next(
...
class TestResilientClient:
    def test_circuit_breaker_annotation(
        self, collector: JavaAstCollector
    ) -> None:
        results = collector.collect(FIXTURES, config=None)
        resilient = next(
            e for e in results if "ResilientClient" in e.payload["file_path"]
        )
...
class TestFaultIsolation:
    def test_malformed_java_skipped_with_warning(
        ...
        good_ev = next(
            (e for e in results if "Good" in e.payload["file_path"]), None
        )
        assert good_ev is not None

    def test_empty_java_file(
...
class TestLogStatementExtraction:
    def test_extracts_logger_calls(
        self, collector: JavaAstCollector, tmp_path: Path
    ) -> None:
